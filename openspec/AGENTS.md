...
2. 选择唯一的动词引导的 `change-id`，并在 `openspec/changes/<id>/` 下搭建 `proposal.md`、`tasks.md`、可选的 `design.md` 和规范增量。
3. 使用 `## ADDED|MODIFIED|REMOVED Requirements` 起草规范增量，每个需求至少包含一个 `#### Scenario:`。
4. 在共享提案之前运行 `openspec validate <id> --strict` 并解决任何问题。
...
1. **创建目录：** `changes/[change-id]/`（kebab-case 格式，动词引导，唯一）
...
- RENAMED：仅当名称更改时使用。如果您还更改了行为，使用 RENAMED（名称）加上引用新名称的 MODIFIED（内容）。

常见陷阱：使用 MODIFIED 添加新关注点而不包括先前的文本。这会导致归档时丢失详细信息。如果您没有明确更改现有需求，请改为在 ADDED 下添加新需求。
...
**静默场景解析失败**
- 需要精确格式：`#### Scenario: 名称`
- 调试方法：`openspec show [change] --json --deltas-only`
...
### CLI 要点
```bash
openspec list              # 进行中的是什么？
openspec show [item]       # 查看详细信息
openspec validate --strict # 是否正确？
openspec archive <change-id> [--yes|-y]  # 标记完成（添加 --yes 以进行自动化）
```
